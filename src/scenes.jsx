...
const ease = (frame, fps, delay = 0, damping = 200) =>
...
export const Hook = ({text, accent, head}) => {
  ...
  return (
    <div style={{...center, padding: '0 90px'}}>
      {head ? (
        <div style={{...kicker, marginBottom: 44}}>{head}</div>
      ) : null}
...
export const Rank = ({to = 340, total = 400, caption = 'applicants'}) => {
  ...
  const pos = interpolate(frame, [0, settle], [0, to], {
    extrapolateRight: 'clamp',
    easing: (t) => 1 - Math.pow(1 - t, 4), // fast start, long decelerate
  });
  ...
  return (
    <div style={{...center, justifyContent: 'flex-start', paddingTop: 460}}>
      {/* counter sits above the stack, not beside it — beside it the kicker
          collides with the sheets */}
      ...
      <div style={{position: 'relative', width: 700, height: 900, overflow: 'hidden'}}>
        ...
        {/* the stack fades out top and bottom instead of hard-clipping */}
        <div
          style={{
            position: 'absolute',
            inset: 0,
...
// ---------------------------------------------------------------- stat
// Counts up, then names its source. The source chip is not decoration — the
// campaign rule is that no number appears on screen without one.
      ...
      {source ? (
        <div
          style={{
            ...chip,
            fontSize: 26,
            whiteSpace: 'nowrap', // a wrapped source chip leaves an orphan line
            marginTop: 54,
...
// running is the sample, not a teaser.
export const Prompt = ({title, lines = []}) => {
  const frame = useCurrentFrame();
  const {durationInFrames} = useVideoConfig();
  const body = lines.join('\n');
  // Size the type to the longest line so nothing wraps — a wrapped prompt reads
  // as broken code. Mono glyphs are ~0.6em wide.
...
// returns a bad result about their own resume. Costs nothing to act on, and a
// bad result is what makes the free pack worth downloading — the prompt scenes
// prove the product is good, this one proves they have the problem.
...
// ---------------------------------------------------------------- cta
// Same mark as the profile picture, drawn in CSS so it animates.
//
// `keyword` turns this into a comment-keyword CTA. That matters: "link in bio"
// costs a viewer four taps, while a comment is one tap, is itself engagement,
// and on Instagram a bot can DM the link back automatically.
...
export const SCENES = {hook: Hook, rank: Rank, stat: Stat, prompt: Prompt, test: Test, cta: Cta};
